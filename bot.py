...
STALE_AFTER_HOURS = int(os.environ.get("STALE_AFTER_HOURS", "48"))
...
async def check_paid_status(client: httpx.AsyncClient, address: str) -> bool:
    ...
    if resp.status_code != 200:
        ...
        return False
    orders = resp.json()
    return any(o.get("status") == "approved" for o in orders)
...
async def list_watched(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not watchlist:
        ...
        return
    lines = []
    for address, info in watchlist.items():
        age_hours = (time.time() - info["added_at"]) / 3600
        lines.append(f"• {info.get('label', address)}\n  `{address}` — watching {age_hours:.1f}h")
